...
// Route to get server usage
app.get('/usage', async (req, res) => {
  try {
    const [cpuUsage, mem, disk, networkStats, processes, currentLoad, diskIO, networkInterfaces, cronJobs] = await Promise.all([
    ...
    const data = {
      ...
      cpu: cpuUsage.cpu,
      memory: bytesToGB(cpuUsage.memory),
      uptime: os.uptime(),
      loadavg: os.loadavg(),
      totalMemory: bytesToGB(mem.total),
      freeMemory: bytesToGB(mem.available),
      usedMemory: bytesToGB(mem.used),
      utilization,
      memory,
      diskInfo: disk.map(d => ({
        filesystem: d.fs,
