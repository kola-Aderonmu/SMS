...
function Detail({label, value, measurement, className}) {
  return <div className="flex flex-col items-start">
    ...
    <p className={`text-md font-bold pl-1 ${className}`}>{value}{measurement}</p>
  </div>
  }
...
export default function CPUUtilization() {
  ...
  const {baseSpeed, brand, cache, logicalProcessor, manufacturer, physicalCores, processes, socket, speed, uptime, virtualization} = cpuMetrics;
  ...
  const formattedTime = moment.utc(duration.asMilliseconds()).format('D:HH:mm:ss.SS');
    ...
    <div>
      <div className="flex items-center justify-between px-3 gap-2">
          ...
          <p className="text-xs font-light">
            {manufacturer}{brand} CPU @ {speed.toFixed(2)}GHz
          </p>
      </div>
      ...
      <div className="flex flex-row justify-start gap-20  p-3">
        ...
        <Detail label={"Uptime"} value={formattedTime} className={"font-light text-xs"} />
        </div>
        <div className="flex flex-col gap-2">
        <RowDetail label={"Base Speed"} value={baseSpeed} measurement={"GHz"} />
        ...
        <RowDetail label={"Virtualization"} value={virtualization ? "Enabled": "Disabled"}  />
        <RowDetail label={"L1 Cache"} value={cache.l1i} measurement={"kB"} />
        <RowDetail label={"L2 Cache"} value={cache.l2 / (1024**2)} measurement={"MB"} />
        <RowDetail label={"L3 Cache"} value={cache.l3 / (1024**2)} measurement={"MB"} />
